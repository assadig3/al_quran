...
class SurahListFragment : Fragment() {
        ...
        search.addTextChangedListener(object : TextWatcher {
            ...
            override fun onTextChanged(s: CharSequence?, start: Int, before: Int, count: Int) {
                ...
                val filtered = if (q.isEmpty()) {
                    ...
                    allSurahs.filter { it.name.contains(q) }
                } else {
                    allSurahs.filter { it.englishName.contains(q, ignoreCase = true) }
                }
    ...
    private fun showJuzPickerDialog() {
        ...
        val locale = requireContext().resources.configuration.locales[0]
        val namesAr = resources.getStringArray(R.array.juz_names) // أو أنشئها يدويًا
